...
export default async function Page({ params }: { params: Params }) {
  ...
  /**
   * Because we want to show the navigation slice on it's own at the top/bottom of the page, we need to split it out of the array that contains all of the slices.
   * We do this by checking the `slice_type` on each slice in the reduce function below.
   */
  const { navigation, otherElements } = slices.reduce<{
    navigation: NavigationSlice[];
    otherElements: RichTextSlice[];
  }>(
    (acc, cur) => {
      if (cur.slice_type === 'navigation') {
        acc.navigation.push(cur);

        return acc;
      }

      acc.otherElements.push(cur);
  ...
  return (
    <div className="flex flex-col gap-12 w-full max-w-3xl">
      ...
      <SliceZone slices={otherElements} components={components} />
